...
void rxs::FindSharesKernel::enqueue(cl_command_queue queue, size_t threads)
{
...
void rxs::FindSharesKernel::setArgs(cl_mem hashes, cl_mem shares)
{
...
void rxs::FindSharesKernel::setTarget(uint64_t target)
{
    setArg(1, sizeof(uint64_t), &target);
}


void rxs::FindSharesKernel::setNonce(uint32_t nonce)
{
