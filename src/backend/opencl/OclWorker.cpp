...
rxs::OclWorker::OclWorker(size_t id, const OclLaunchData &data) :
...
rxs::OclWorker::~OclWorker()
{
    delete m_runner;
}


void rxs::OclWorker::jobEarlyNotification(const Job &job)
{
    if (m_runner) {
...
bool rxs::OclWorker::selfTest()
{
    return m_runner != nullptr;
}


size_t rxs::OclWorker::intensity() const
{
    return m_runner ? m_runner->roundSize() : 0;
}


void rxs::OclWorker::start()
{
...
void rxs::OclWorker::storeStats(uint64_t t)
{
    if (!isReady()) {
        return;
    }
