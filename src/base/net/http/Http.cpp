...
bool rxs::Http::isEqual(const Http &other) const
{
...
rapidjson::Value rxs::Http::toJSON(rapidjson::Document &doc) const
{
...
void rxs::Http::load(const rapidjson::Value &http)
{
    if (!http.IsObject()) {
        return;
    }
...
void rxs::Http::setPort(int port)
{
