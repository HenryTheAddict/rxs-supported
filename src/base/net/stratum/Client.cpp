...
rxs::Client::Client(int id, const char *agent, IClientListener *listener) :
...
const char *rxs::Client::tlsFingerprint() const
{
#   ifdef RXS_FEATURE_TLS
...
const char *rxs::Client::tlsVersion() const
{
#   ifdef RXS_FEATURE_TLS
    if (isTLS()) {
...
int64_t rxs::Client::send(const rapidjson::Value &obj, Callback callback)
{
...
int64_t rxs::Client::send(const rapidjson::Value &obj)
{
...
int64_t rxs::Client::submit(const JobResult &result)
{
...
void rxs::Client::connect(const Pool &pool)
{
    setPool(pool);
    connect();
}
...
void rxs::Client::tick(uint64_t now)
{
...
void rxs::Client::onResolved(const DnsRecords &records, int status, const char *error)
{
    m_dns.reset();
...
bool rxs::Client::parseJob(const rapidjson::Value &params, int *code)
{
    if (!params.IsObject()) {
        *code = 2;
        return false;
...
bool rxs::Client::verifyAlgorithm(const Algorithm &algorithm, const char *algo) const
{
...
bool rxs::Client::write(const uv_buf_t &buf)
{
...
int rxs::Client::resolve(const String &host)
{
...
int64_t rxs::Client::send(size_t size)
{
...
void rxs::Client::connect(const sockaddr *addr)
{
...
bool rxs::Client::parseLogin(const rapidjson::Value &result, int *code)
{
...
void rxs::Client::parse(char *line, size_t len)
{
...
void rxs::Client::parseExtensions(const rapidjson::Value &result)
{
    m_extensions.reset();
...
void rxs::Client::parseNotification(const char *method, const rapidjson::Value &params, const rapidjson::Value &)
{
...
void rxs::Client::parseResponse(int64_t id, const rapidjson::Value &result, const rapidjson::Value &error)
{
...
void rxs::Client::ping()
{
    send(snprintf(m_sendBuf.data(), m_sendBuf.size(), "{\"id\":%" PRId64 ",\"jsonrpc\":\"2.0\",\"method\":\"keepalived\",\"params\":{\"id\":\"%s\"}}\n", m_sequence.load(), m_rpcId.data()));
}


void rxs::Client::read(ssize_t nread, const uv_buf_t *buf)
{
...
void rxs::Client::setState(SocketState state)
{
...
bool rxs::Client::isCriticalError(const char *message)
{
    if (!message) {
        return false;
    }
...
void rxs::Client::onClose(uv_handle_t *handle)
{
...
void rxs::Client::onConnect(uv_connect_t *req, int status)
{
...
void rxs::Client::onRead(uv_stream_t *stream, ssize_t nread, const uv_buf_t *buf)
{
